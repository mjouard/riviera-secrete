...
plus "Créer un itinéraire" and "Mes itinéraires" (→ the two root-level pages below); every
other page (`lieux/*.html`, `itin/*.html`, `credits.html`) has "Itinéraires" (→
`../index.html#itineraires`), "Lieux" (→ `../index.html#lieux`) and the same two builder
links (one `../` level up) — no "Carte" equivalent to link to from a subpage, so that one was
dropped rather than repointed. Itinéraire page breadcrumbs go straight `Accueil → [titre]`,
no middle "Itinéraires" crumb. The nav `<a>` markup is duplicated in 4 places — `index.html`,
`credits.html`, and the `<nav>` block inside `scripts/render/lieu.mjs` and
`scripts/render/itin.mjs` — change all 4 together and rebuild.
...
- **`assets/itineraire-builder.js`** — controller for `creer-itineraire.html`: renders the
  ...
  affine lieu par lieu" behavior, no separate zone/lieu selection model needed), runs
  `generateItineraire` on click, and renders day columns with native HTML5 drag-and-drop
  (`draggable`, live DOM reorder on `dragover` via the standard "closest midpoint" pattern,
  re-synced into the in-memory `currentDays` state on `dragend`) — no drag library, per the
  site's no-framework constraint. `?id=<uuid>` in the URL loads a saved itinerary back in
  (pre-checks the picker, jumps straight to the results view, and "Sauvegarder" then updates
  that same entry instead of creating a new one).
