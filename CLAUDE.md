...
need updating if the lieu count changes again: `ROADMAP.md`'s placeholder-image tally.
`home-lieux.mjs`'s `buildItemList` and `build.mjs`'s sitemap generator both use
`riviera-secrete.vercel.app` now (fixed 2026-09-11 — they briefly disagreed, one still on
`netlify.app` post-migration). That's distinct from `lieu.mjs`/`ville.mjs`'s own
canonical/`og:url`/JSON-LD tags and `data/lieux.json`'s `ogImage` fields, which **still
hardcode `netlify.app`** — left alone, since fixing those is the pre-existing, deliberately
deferred `ROADMAP.md` task ("Mettre à jour toutes les URLs canoniques") to do once when a
real custom domain is bought, not before.
...
client/server app — the priority stack for ongoing work (decided 2026-09-11, see "What this
is" above), not yet cut over as the production domain. The motivation is accounts: the
static site has no server to hold user data, so localStorage is the ceiling for anything
personal (favoris, itinéraires custom). This stack exists to lift that ceiling. `ROADMAP.md`
points to a memory file `architecture_future.md` for the architecture decision behind
this — it does exist, in the repo's own `.claude/memory/` (not this file), along with
`backend_dotnet.md`, `frontend_nextjs.md` and `frontend_migration_checklist.md`, all more
current/detailed on this stack's day-to-day state than this section — check those first,
fix this section to match if they disagree.
...
- **Entities** (`RivieraSecrete.Domain/Entities/`): `Ville` 1–N `Lieu` 1–N `Activite`
  ...
  `itineraires.json` has for its `related`/`suggestions` arrays. `User` owns `UserFavorite`
  (a user × lieu slug pair) and `UserItineraire` (`Days: string[][]` of lieu slugs,
  `DureeKey` — the DB-backed sibling of the static site's
  `riviera-secrete:itineraires-custom` localStorage schema, same shape). `User` supports two
...
- **Migrations**: `InitialCreate` (villes/lieux/activités/itinéraires schema), `AddUserTables`
  ...
  not automated on startup. **Building/migrating this project needs the .NET 10 SDK**
  (`TargetFramework net10.0`) — a machine with only up to .NET 8 installed (check `dotnet
  --list-sdks`) needs it installed first (e.g. `dotnet-install.sh --channel 10.0
  --install-dir ~/.dotnet`, since writing to the system-wide `/usr/local/share/dotnet`
  needs sudo); add `~/.dotnet` and `~/.dotnet/tools` to `PATH` and set `DOTNET_ROOT`.
...
`/connexion` and `/confirmer-email` (added 2026-09-12, see below), plus
...
no localStorage fallback — logged-out visitors are linked to `/connexion?callbackUrl=…`),
unlike `/mes-itineraires`'s DB-with-localStorage-fallback hybrid described above for
logged-out users. Every entry point that used to call `signIn("google")` directly
(`NavHeader`, `FavoriteButton`, `creer-itineraire`, `mes-itineraires`, `mes-favoris`) now
links to `/connexion?callbackUrl=…` instead, so the visitor picks Google vs. email/password
rather than being dropped straight into Google's OAuth redirect.
...
- **`/confirmer-email`** — reads `?token=` from the URL (client-side, same
  `window.location.search` pattern as `creer-itineraire`'s `?id=`, not `useSearchParams`, to
  avoid a Suspense boundary), POSTs it to `/api/auth/confirm-email`, shows success (link to
  `/connexion`) or the specific backend error (e.g. expired token).
...
- **`src/lib/api.ts`** — typed fetch helpers against the backend: `api.{lieux,villes,
  ...
  through `decodeDeep()` (added 2026-09-12), which walks the parsed JSON recursively and
  decodes HTML entities (`&amp;` → `&`, etc.) on every string leaf via
  `decodeEntities()` in `src/lib/utils.ts` — needed because `data/lieux.json`/
  `itineraires.json` contain literal `&amp;` written for the static site's raw-HTML
  rendering (`scripts/render/*.mjs`), which React would otherwise double-escape and display
  verbatim as `&amp;` when shown as JSX text. Pure string logic, no DOM — safe in Server
  Components too.
- **`src/lib/auth.ts`** — NextAuth v4 config: `GoogleProvider` **and** `CredentialsProvider`
  (added 2026-09-12) side by side, `pages.signIn` pointed at `/connexion`. The `jwt` callback
  branches on which provider fired: Google still does the `account.id_token` →
  `POST /api/auth/google-signin` exchange described above; the credentials path's
  `authorize()` instead calls `POST /api/auth/login` itself and returns
  `{ id, email, name, apiToken }` on success (`null` on any failure, per NextAuth
  convention), which the `jwt` callback picks up via the `user` argument. Either path ends
  up stashing `{ apiToken, apiUser }` onto the NextAuth JWT; the `session` callback then
  surfaces those as `session.apiToken` and `session.user.{id,name,email}` so client
  components never touch NextAuth's own token shape directly.
  `frontend/src/types/next-auth.d.ts` extends the library's `Session`/`JWT`/**`User`**
  types to add these fields (the `User.apiToken` extension is what lets `authorize()`'s
  return value carry the backend JWT through to the `jwt` callback).
...
- **Leaflet maps**: `LeafletLieuMap`/`LeafletItinMap`/`HomeMap`/`BuilderMap` are the actual
  ...
  fixed 2026-09-12) — they originally pointed at
  `basemaps.cartocdn.com/dark_all`, which CARTO has since locked behind a required API key
  (confirmed by fetching a tile directly: it returns a 200 OK PNG, but the image itself is a
  literal "API KEY REQUIRED — carto.com/basemaps/apikey" placeholder, not a map, so this
  silently "worked" per an HTTP-status check while rendering nothing real). `map-tiles.ts`'s
  `applyDarkTileFilter(map)` applies `invert(1) hue-rotate(180deg) brightness(0.95)
  contrast(0.9) saturate(0.3)` to the map's `tilePane` to approximate the site's dark theme
  on top of OSM's light tiles. When adding a `useEffect`-based Leaflet init (the pattern all
  of these follow: `import("leaflet").then(L => { ... })`), guard against React Strict
  Mode's double-invoke in dev with a `cancelled` flag checked at the top of the `.then()`
  callback and set in the effect's cleanup — `HomeMap.tsx` has this guard (added when the
  "Map container is already initialized" error surfaced there 2026-09-11), but
  `LeafletLieuMap.tsx`/`LeafletItinMap.tsx` still don't, so the same latent bug likely
  exists on lieu/itinéraire pages too — apply the same fix there if it surfaces.
- **Homepage** (`src/app/page.tsx`) — as of 2026-09-11 renders four sections beyond the
  hero: itinéraire cards (now with a thumbnail from the first stop's lieu, via a
  `lieuBySlug` lookup built in `page.tsx` — this was missing before that date),
  `HomeActivities` (tabbed by category — outdoor/culture/gastronomie/loisirs — using a
  `FEATURED_ACTIVITIES` curation map in `src/lib/home-data.ts` copied from the static site's
  own inline curation in `index.html`; keep both in sync), `HomeMapWrapper` (Leaflet map of
  the 22 villes, region-colored markers, click → info card, region filter buttons — port of
  the static site's `#carte` section), and `HomeLieuxGrid` (the 27-lieu grid with the same 5
  badge filters — `plage`/`randonnee`/`vtt`/`plongee`/`restaurant` — as the static site,
  `BADGE_DEFS` duplicated in `home-data.ts`). All four were absent on the Next.js homepage
  until 2026-09-11 despite existing on the static site; `frontend_migration_checklist.md`
  (repo memory) has the fuller feature-by-feature parity table.
...
- **Deploying**: frontend via `cd frontend && vercel --prod --yes` (see
  `feedback_vercel_deploy.md` in the repo's `.claude/memory/` for the "never from the repo
  root" pitfall); backend via `cd backend && railway up --service api` (needs `railway link
  --project fearless-happiness --service api --environment production` first on a machine
  that's never linked it). Both CLIs need an interactive browser login the first time on a
  new machine (`vercel login` / `railway login`, device-code flow) — cannot be done
  non-interactively, has to be a human completing it in a browser.
