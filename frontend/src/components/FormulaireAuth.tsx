 ...
 *
 * Refonte UI Lot 2 — le mode était un état local basculé par un bouton, sans changement
 * d'URL : impossible d'envoyer un lien direct vers l'inscription, et rien à mesurer côté
 * entonnoir. C'est désormais une **prop**, fixée par la route qui monte le composant ; les
 * deux pages partagent cette implémentation unique plutôt qu'une copie chacune.
 *
 * La demande de réinitialisation de mot de passe, elle aussi coincée ici en troisième état,
 * a sa propre route (/mot-de-passe-oublie) et n'est plus qu'un lien.
 */
export default function FormulaireAuth({ mode }: { mode: Mode }) {
  ...
  /** Le `callbackUrl` doit survivre au passage d'un formulaire à l'autre. */
